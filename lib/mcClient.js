...
class MCClient {
    ...
    static getMessage(topic, to, payload) {
        let isToExist = to && to != ''
        let isTopicExist = topic && topic != ''
        let isBothExist = isToExist && isTopicExist
        return {
...
module.exports = {
    ...
    send(topic, payload) {
        return new Promise((resolve, reject) => {
            ...
            const [proto, host] = dealTopic(topic)
            MCClient.setEiName(os.hostname()).then(() => {
                const message = MCClient.getMessage(proto, host, payload)
     ...
     */
    call(topic, func, payload = {}) {
        const [proto, host] = dealTopic(topic)
        return new Promise((resolve, reject) => {
            MCClient.setEiName(os.hostname()).then(() => {
                let xrpc = MCClient.getRPC(proto, host, func, payload)
